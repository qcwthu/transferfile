...
do
        ...
        do
                ...
                do
			CHECKPOINT="models/upstream-base-multitask-cls2cls-"$onelr"-"$onebs"-"$oneepoch"/last-model.pt"
                        IDENTIFIER="T5-"$size"-multitask-cls2cls-"$onelr"-"$onebs"-"$oneepoch
			for TASK in ${TASKS[@]}
			do
  				echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
  				python -m torch.distributed.launch --nproc_per_node 2 --master_port 36226 singletask_from_multi_cls2cls.py \
        			--task_dir data/${TASK}/ \
        			--task_name ${TASK} \
