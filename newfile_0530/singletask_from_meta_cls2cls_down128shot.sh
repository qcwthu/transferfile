...
TASKS=("superglue-cb" "dbpedia_14" "wiki_qa" "emo" "yelp_polarity" "ethos-religion" "amazon_polarity" "tab_fact")
...
do
        ...
        do
                ...
                do
                        ...
                        do
				CHECKPOINT="models/upstream-maml-cls2cls-"$onelr"-"$oneg"-"$onestep"-"$outerlr"/last-model.pt"
				IDENTIFIER="T5-"$size"-maml-cls2cls-"$onelr"-"$oneg"-"$onestep"-"$outerlr"-down128shot"
				for TASK in ${TASKS[@]}
				do
  				echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
 		 		python -m torch.distributed.launch --nproc_per_node 2 --master_port 29479 singletask_from_meta_cls2cls.py \
