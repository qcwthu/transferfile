...
class FewshotGymClassificationDataset(FewshotGymDataset):
    ...
    def generate_k_shot_data(self, k, seed, path=None):
        ...
        ######for superglue-cb dataset
        ######superglue-cb
        ######superglue-cb
        ######superglue-cb
        tousenum = {}
        for label in label_list:
            if len(label_list[label]) >= 2 * k:
                tousenum[label] = k
            else:
                tousenum[label] = len(label_list[label]) // 2
        #print(tousenum)

        k_shot_train = []
        for label in label_list:
            #print(label, len(label_list[label]))
            #for line in label_list[label][:k]:
            for line in label_list[label][:tousenum[label]]:
                k_shot_train.append(line)

        k_shot_dev = []
        for label in label_list:
            #for line in label_list[label][k:2*k]:
            for line in label_list[label][tousenum[label]:2 * tousenum[label]]:
