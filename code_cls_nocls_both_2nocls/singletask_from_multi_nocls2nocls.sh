...
do
        ...
        do
                ...
                do
                      CHECKPOINT="models/upstream-multitask-nocls2nocls-"$onelr"-"$onebs"-"$oneepoch"/last-model.pt"
                      IDENTIFIER="T5-"$size"-multitask-nocls2nocls-"$onelr"-"$onebs"-"$oneepoch
                      for TASK in ${TASKS[@]}
                      do
                          echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
                          python -m torch.distributed.launch --nproc_per_node 2 --master_port 26131 singletask_from_multi_nocls2nocls.py \
                              --task_dir data/${TASK}/ \
                              --task_name ${TASK} \
                              ...
                              --prompt_number 100
                            echo "++++++++++++++++++++++++++++++"
                            ps aux | grep singletask_from_multi_nocls2nocls.py | awk '{print $2}' | xargs kill -9
			                done
		            done
        done
done
