...
CHECKPOINT="None"
size="large"
IDENTIFIER="T5-"$size"-ft-nonli2nli"

for TASK in ${TASKS[@]}
do
  echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
  python -m torch.distributed.launch --nproc_per_node 2 --master_port 21356 tune_singletask_nonli2nli.py \
        --task_dir data/${TASK}/ \
        --task_name ${TASK} \
  ...
  echo "++++++++++++++++++++++++++++++"
	ps aux | grep tune_singletask_nonli2nli.py | awk '{print $2}' | xargs kill -9
done
