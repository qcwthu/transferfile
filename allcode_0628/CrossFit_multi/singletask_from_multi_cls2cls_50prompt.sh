...
do
        ...
        do
                ...
                do
			CHECKPOINT="models/upstream-50prompt-multitask-cls2cls-"$onelr"-"$onebs"-"$oneepoch"/last-model.pt"
                        IDENTIFIER="T5-"$size"-multitask-cls2cls-"$onelr"-"$onebs"-"$oneepoch"-50prompt"
			for TASK in ${TASKS[@]}
			do
  				echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
  				python -m torch.distributed.launch --nproc_per_node 2 --master_port 37131 singletask_from_multi_cls2cls.py \
        			--task_dir data/${TASK}/ \
        			--task_name ${TASK} \
