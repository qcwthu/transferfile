...
do
        ...
        do
                ...
                do
			CHECKPOINT="models/upstream-multitask-cls2cls-"$onelr"-"$onebs"-"$oneepoch"-128shot/last-model.pt"
                        IDENTIFIER="T5-"$size"-multitask-cls2cls-"$onelr"-"$onebs"-"$oneepoch"-up128shot"
			for TASK in ${TASKS[@]}
			do
  				echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
  				python -m torch.distributed.launch --nproc_per_node 2 --master_port 26259 singletask_from_multi_cls2cls.py \
        			--task_dir data/${TASK}/ \
        			--task_name ${TASK} \
