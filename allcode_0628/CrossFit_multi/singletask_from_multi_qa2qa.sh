...
do
        ...
        do
                ...
                do
			CHECKPOINT="models/upstream-multitask-qa2qa-"$onelr"-"$onebs"-"$oneepoch"/last-model.pt"
                        IDENTIFIER="T5-"$size"-multitask-qa2qa-"$onelr"-"$onebs"-"$oneepoch
			for TASK in ${TASKS[@]}
			do
  				echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
  				python -m torch.distributed.launch --nproc_per_node 2 --master_port 26237 singletask_from_multi_qa2qa.py \
        			--task_dir data/${TASK}/ \
        			--task_name ${TASK} \
        			...
        			--model google/t5-v1_1-$size \
 			        --prompt_number 100
     				echo "++++++++++++++++++++++++++++++"
      			ps aux | grep singletask_from_multi_qa2qa.py | awk '{print $2}' | xargs kill -9
			done
		done
        done
done
