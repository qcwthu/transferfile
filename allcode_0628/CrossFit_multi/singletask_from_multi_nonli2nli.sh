...
do
        ...
        do
                ...
                do
                      CHECKPOINT="models/upstream-multitask-nonli2nli-"$onelr"-"$onebs"-"$oneepoch"/last-model.pt"
                      IDENTIFIER="T5-"$size"-multitask-nonli2nli-"$onelr"-"$onebs"-"$oneepoch
                      for TASK in ${TASKS[@]}
                      do
                          echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
                          python -m torch.distributed.launch --nproc_per_node 2 --master_port 28414 singletask_from_multi_nonli2nli.py \
                              --task_dir data/${TASK}/ \
                              --task_name ${TASK} \
                              ...
                              --prompt_number 100
                            echo "++++++++++++++++++++++++++++++"
                            ps aux | grep singletask_from_multi_nonli2nli.py | awk '{print $2}' | xargs kill -9
			                done
		            done
        done
done
