...
do
        ...
        do
			  ...
			  for onerep in ${allreptilestep[@]}
                          do
				CHECKPOINT="models/upstream-reptile-cls2cls-"$onelr"-"$oneg"-"$onestep"-"$outerlr"-"$onerep"/last-model.pt"
				...
				do
 		 		...
 		 		python -m torch.distributed.launch --nproc_per_node 2 --master_port 26556 singletask_from_reptile_cls2cls.py \
        			--task_dir data/${TASK}/ \
        			--task_name ${TASK} \
