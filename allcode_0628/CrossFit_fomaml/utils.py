...
def getpromptembedding(model,tokenizer,promptnumber,train_tasks):
    ...
    startindex = 0
    alltasks = []
    for onetask in train_tasks:
        newname = " ".join(re.split('_|-',onetask))
        alltasks.append(newname)
    for one in alltasks:
        ...
        startindex += 1
        if startindex >= promptnumber:
            print("initialize finish early")
    ...
    print("initialize from c4")
    randomtokennum = promptnumber - len(alltasks)
