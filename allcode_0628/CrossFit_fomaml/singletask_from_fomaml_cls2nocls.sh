...
do
        ...
        do
                ...
                do
                        ...
                        do
				CHECKPOINT="models/upstream-fomaml-cls2nocls-"$onelr"-"$oneg"-"$onestep"-"$outerlr"/last-model.pt"
				IDENTIFIER="T5-"$size"-fomaml-cls2nocls-"$onelr"-"$oneg"-"$onestep"-"$outerlr
				for TASK in ${TASKS[@]}
				do
  				echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
 		 		python -m torch.distributed.launch --nproc_per_node 2 --master_port 24574 singletask_from_fomaml_cls2nocls.py \
        			--task_dir data/${TASK}/ \
        			--task_name ${TASK} \
				...
				--learning_rate_list 5e-1 4e-1 3e-1 2e-1 \
        			...
        			--prompt_number 100
      				echo "++++++++++++++++++++++++++++++"
      				ps aux | grep singletask_from_fomaml_cls2nocls.py | awk '{print $2}' | xargs kill -9
				done
			 done
                done
        done
