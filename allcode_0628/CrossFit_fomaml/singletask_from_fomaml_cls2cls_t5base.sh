...
do
        ...
        do
                ...
                do
                        ...
                        do
				CHECKPOINT="models/upstream-base-fomaml-cls2cls-"$onelr"-"$oneg"-"$onestep"-"$outerlr"/last-model.pt"
				IDENTIFIER="T5-"$size"-fomaml-cls2cls-"$onelr"-"$oneg"-"$onestep"-"$outerlr
				for TASK in ${TASKS[@]}
				do
  				echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
 		 		python -m torch.distributed.launch --nproc_per_node 2 --master_port 35569 singletask_from_fomaml_cls2cls.py \
        			--task_dir data/${TASK}/ \
        			--task_name ${TASK} \
