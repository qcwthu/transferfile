...
do
        ...
        do
                ...
                do
                        ...
                        do
                            CHECKPOINT="models/upstream-fomaml-both2cls-"$onelr"-"$oneg"-"$onestep"-"$outerlr"/last-model.pt"
                            IDENTIFIER="T5-"$size"-fomaml-both2cls-"$onelr"-"$oneg"-"$onestep"-"$outerlr
                            for TASK in ${TASKS[@]}
                            do
                              echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
                              python -m torch.distributed.launch --nproc_per_node 2 --master_port 24571 singletask_from_fomaml_both2cls.py \
                                  --task_dir data/${TASK}/ \
                                  --task_name ${TASK} \
                                  ...
                                  --prompt_number 100
                                  echo "++++++++++++++++++++++++++++++"
                                  ps aux | grep singletask_from_fomaml_both2cls.py | awk '{print $2}' | xargs kill -9
				                    done
			                done
                done
        done
