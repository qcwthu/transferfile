...
do
        ...
        do
                ...
                do
                        ...
                        do
				CHECKPOINT="models/upstream-maml-cls2cls-"$onelr"-"$oneg"-"$onestep"-"$outerlr"-128shot/last-model.pt"
				IDENTIFIER="T5-"$size"-maml-cls2cls-"$onelr"-"$oneg"-"$onestep"-"$outerlr"-up128shot"
				for TASK in ${TASKS[@]}
				do
  				echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
 		 		python -m torch.distributed.launch --nproc_per_node 2 --master_port 25278 singletask_from_meta_cls2cls.py \
        			--task_dir data/${TASK}/ \
        			--task_name ${TASK} \
