...
TASKS=("qasc" "quail" "quarel" "quartz-no_knowledge" "quartz-with_knowledge" "sciq" "superglue-copa" "swag" "wino_grande" "wiqa")
...
do
        ...
        do
                ...
                do
                        ...
                        do
				CHECKPOINT="models/upstream-maml-qa2qa-"$onelr"-"$oneg"-"$onestep"-"$outerlr"/last-model.pt"
				IDENTIFIER="T5-"$size"-maml-qa2qa-"$onelr"-"$oneg"-"$onestep"-"$outerlr
				for TASK in ${TASKS[@]}
				do
  				echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
 		 		  python -m torch.distributed.launch --nproc_per_node 2 --master_port 29650 singletask_from_meta_qa2qa.py \
        			--task_dir data/${TASK}/ \
        			--task_name ${TASK} \
        			...
        			--prompt_number 100
      				echo "++++++++++++++++++++++++++++++"
      				ps aux | grep singletask_from_meta_qa2qa.py | awk '{print $2}' | xargs kill -9
				done
			 done
                done
        done
