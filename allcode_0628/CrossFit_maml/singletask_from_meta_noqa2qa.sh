...
#TASKS=("sciq" "superglue-copa" "swag" "wino_grande" "wiqa")

#TASKS=("quail")
TASKS=("quarel" "quartz-no_knowledge" "quartz-with_knowledge")
allinnerlr=(3e-5)
allgradient=(2)
allstep=(5000)
alloutlr=(5e-1)
...
do
        ...
        do
                ...
                do
                        ...
                        do
                          CHECKPOINT="models/upstream-maml-noqa2qa-"$onelr"-"$oneg"-"$onestep"-"$outerlr"/last-model.pt"
                          IDENTIFIER="T5-"$size"-maml-noqa2qa-"$onelr"-"$oneg"-"$onestep"-"$outerlr
                          for TASK in ${TASKS[@]}
                          do
                            echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
                            python -m torch.distributed.launch --nproc_per_node 2 --master_port 29651 singletask_from_meta_noqa2qa.py \
                                --task_dir data/${TASK}/ \
                                --task_name ${TASK} \
                                ...
                                --prompt_number 100
                                echo "++++++++++++++++++++++++++++++"
                                ps aux | grep singletask_from_meta_noqa2qa.py | awk '{print $2}' | xargs kill -9
