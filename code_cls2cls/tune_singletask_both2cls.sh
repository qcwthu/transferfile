...
CHECKPOINT="None"
size="large"
IDENTIFIER="T5-"$size"-ft-both2cls"

for TASK in ${TASKS[@]}
do
  echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
  python -m torch.distributed.launch --nproc_per_node 2 --master_port 24569 tune_singletask_both2cls.py \
        --task_dir data/${TASK}/ \
        --task_name ${TASK} \
  ...
  echo "++++++++++++++++++++++++++++++"
	ps aux | grep tune_singletask_both2cls.py | awk '{print $2}' | xargs kill -9
done
