...
IDENTIFIER="T5-"$size"-nocls2cls"

for TASK in ${TASKS[@]}
do
  echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
  python -m torch.distributed.launch --nproc_per_node 2 --master_port 29112 tune_hps_singletask_ddp_prompt_nocls2cls.py \
        --task_dir data/${TASK}/ \
        --task_name ${TASK} \
        ...
        --prompt_number 100
      echo "++++++++++++++++++++++++++++++"
      ps aux | grep tune_hps_singletask_ddp_prompt_nocls2cls.py | awk '{print $2}' | xargs kill -9
done
