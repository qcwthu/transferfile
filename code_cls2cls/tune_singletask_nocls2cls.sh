...
CHECKPOINT="None"
size="large"
IDENTIFIER="T5-"$size"-ft-nocls2cls"

for TASK in ${TASKS[@]}
do
  echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
  python -m torch.distributed.launch --nproc_per_node 2 --master_port 24568 tune_singletask_nocls2cls.py \
        --task_dir data/${TASK}/ \
        --task_name ${TASK} \
  ...
  echo "++++++++++++++++++++++++++++++"
	ps aux | grep tune_singletask_nocls2cls.py | awk '{print $2}' | xargs kill -9
done
