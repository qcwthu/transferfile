        ...
        do
                ...
                do
                        ...
                        do
                            ...
                            do
                            CHECKPOINT="models/upstream-reptile-nocls2cls-"$onelr"-"$oneg"-"$onestep"-"$outerlr"-"$onerep"/last-model.pt"
                            IDENTIFIER="T5-"$size"-reptile-nocls2cls-"$onelr"-"$oneg"-"$onestep"-"$outerlr"-"$onerep
                            for TASK in ${TASKS[@]}
                            do
                              echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
                              python -m torch.distributed.launch --nproc_per_node 2 --master_port 26557 singletask_from_reptile_nocls2cls.py \
                                  --task_dir data/${TASK}/ \
                                  --task_name ${TASK} \
                                  ...
                                  --prompt_number 100
                                  echo "++++++++++++++++++++++++++++++"
                                  ps aux | grep singletask_from_reptile_nocls2cls.py | awk '{print $2}' | xargs kill -9
                            done
			                  done
			              done
                done
