...
CHECKPOINT="None"
size="large"
IDENTIFIER="Bart-"$size"-ft-nopara2para"

for TASK in ${TASKS[@]}
do
  echo "Task: $TASK, Checkpoint: $CHECKPOINT, Identifier: $IDENTIFIER"
  python -m torch.distributed.launch --nproc_per_node 2 --master_port 21389 tune_singletask_nopara2para_bart.py \
        --task_dir data/${TASK}/ \
        --task_name ${TASK} \
  ...
  echo "++++++++++++++++++++++++++++++"
	ps aux | grep tune_singletask_nopara2para_bart.py | awk '{print $2}' | xargs kill -9
done
